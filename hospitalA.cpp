...
int main(int argc, char* argv[]){
	...
	hospital_server_address.sin_port = htons(30476);
	...
	string client_message_string = string("A:") + to_string(totalCapacity) + "," + to_string(initialOccupancy);
	...
	if(it2 !=mp.end()){
	 ...
	 //    	printf("%d \t\t %f\n", i, shortestDistance[i]);
		...
		if(hospital_location_iterator!=mp.end()){
			...
			printf("Invalid hospital A location\n");
			return -1;
		}
		...
		//Create client message
		if(score!=-1 && d!=-1){
			client_message_string = string("A-Score:") + to_string(score) + ",Distance:" + to_string(d);
		}
		else if(score==-1 && d==-1){
			client_message_string = string("A-None Score,None Distance");
		}
		else if(score==-1){
			client_message_string = string("A-None Score") + ",Distance:" + to_string(d);
		}
		else{
			//d==-1
			client_message_string = string("A-Score:") + to_string(score) + ",None Distance";
		}
		//Convert string to c string
		...
		if(score!=-1 && d!=-1){
			...
			printf("Hospital A has sent score=None and distance=None\n");
		}
		else if(score==-1){
			printf("Hospital A has sent score=None and distance=%f\n",d);
		}
		else{
			//d==-1
			printf("Hospital A has sent score=%f and distance=None\n",score);
		}
		...
		memset(client_message2, '\0', sizeof(client_message2));
		//Create client message
		client_message_string = string("A:Invalid Client location");
			//Convert string to c string
		strcpy(client_message2,client_message_string.c_str());
		// printf("%s\n",client_message2);
